...
Have a go at using our old trusty `predict()` function to calculate the fitted mean bird abundance for each level of `FGRAZE` along with the standard errors. Then plot these fitted values against grazing level and add the 95% confidence intervals (you will need either the `arrows()` or the `segments()` function to add the intervals to the plot). There are also a few packages that will do the whole job for you in one line, and I've put those in the solutions as alternatives, so feel free to use Google (yep, this is OK!) and have a look at the `gplots` package, the `effects` package or `ggplot2` if you fancy it.
...
**Alternative ways of producing the same plot.** All four of these give you essentially the same figure as the one above, so use whichever you find easiest. Don't worry if you can't follow all of the code, none of it is examinable.
...
```{r Q11f, eval=SOLUTIONS, echo=SOLUTIONS, collapse=FALSE}
...
12\. Same idea, but this time I'm not giving you the sentences, only the ingredients. Write a short results paragraph (four or five sentences is plenty) describing the effect of grazing intensity on bird abundance. Make sure it contains:
