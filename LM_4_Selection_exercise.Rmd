...
2\. We assume that all the explanatory variables have been collected by the authors *because* they were believed to be biologically relevant for explaining bird abundance. However, it is a good idea to pause and think what might be relevant or not-so relevant or partly redundant and why, before even exploring the relationships with bird abundance (yes, even graphically). You could do this in a table format, and include a hypothetical ranking of importance. Is there anything that limits your ability to fill such a table?
...
3\. It's useful to start with a graphical exploration of the relationships between explanatory variables and between explanatory variables and the response variable. A pairs plot using the `pairs()` function is a very effective way of doing this when the number of variables is not too large. 
...
5\. Have a look at the summary of the model, and compare the estimates of the coefficients with their standard errors. Are there any which have large SE relative to the coefficient estimate? Do any of these correspond with explanatory variables that you had identified as being potentially collinear?
...
7\. Update the model and repeat single term deletions with `drop1()`, until there are no longer any non-significant terms, ignoring `LOGAREA` , `FGRAZE`, and their interaction (we want to leave them in for now, irrespective of what `drop1` suggests).
...
8\. If all goes well, you should have ended up with the interactive model again `ABUND ~ LOGAREA * FGRAZE`. Do you need to simplify this model? Do you need to use `drop1()` for that?
...
11\. What inferences can we make from this model? What are the biological interpretations, and the statistical lessons you take away from this analysis of the loyn data?
...
12\. Had we not been aiming to test the hypothesis associated with the `LOGAREA * FGRAZE` interaction, we could also have used AIC to perform the model selection. Let's try this (taking the AIC values returned by `AIC` and summarising the performance of the alternative models in a table.
...
```{r Q12c, eval=TRUE, echo=FALSE, results=SOLUTIONS}
