...
# subset the variables of interest. Note we use the numeric GRAZE here,
# not the factor FGRAZE: a correlation coefficient is only meaningful
# between numeric variables, and GRAZE is an ordered index of grazing
# intensity so a correlation with it is interpretable. It is still FGRAZE
...
# might indicate some collinearity, i.e. LOGAREA and GRAZE (0.48), LOGDIST and 
# LOGLDIST (0.59) and YR.ISOL and GRAZE (0.56). However, the relationships
...
# maybe ALT (positive) and GRAZE (negative).
...
# which is quite handy. But the results of this ANOVA are based on sequential 
...
# Summary P values don't suffer from this problem but test different hypotheses.
