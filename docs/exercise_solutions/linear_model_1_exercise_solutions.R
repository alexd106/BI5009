...
# let's first log10 transform the AREA variable due to the couple of unusually large
# forest area values (check you data exploration) to remind yourself
...
# The null hypothesis is that the slope of the relationship between LOGAREA and ABUND =
# 0 i.e. there is no relationship

# The p value is very small (3.81e-14) therefore we reject this null hypothesis (i.e.
...
# the estimate of the intercept = 10.4 the estimate of the slope = 9.78
...
# increase in LOGAREA these data are consistent with an increase of somewhere between
# about 7.8 and 11.8 birds (and a 1 unit increase in LOGAREA is a tenfold increase in
# patch area on the original scale). Our best estimate is 9.78 birds, but it's the
# interval that tells us how precisely we've measured it.
...
# log10(1) = 0), so these data are consistent with somewhere between about 8 and 13
# birds in a one hectare patch.


## ----Q6, eval=SOLUTIONS, echo=SOLUTIONS, collapse=TRUE-----------------------------------------------------------------------------------------------------------------

# the null hypothesis for the intercept is that the intercept = 0 the p value is very
# small (certainly less than the (not so magic) 0.05) therefore we reject this null
# hypothesis and conclude that the intercept is different from zero.

# the null hypothesis for the slope is that the slope = 0 the p value is very small
# (3.81e-14) therefore we reject this null hypothesis and conclude that the slope is
# different from zero.

# VERSION 2, leading with the estimates instead

# Bird abundance increases with forest patch area. We estimate an increase of 9.78 birds
# (95% CI: 7.76 to 11.81) for every 1 unit increase in LOGAREA (i.e. for every tenfold
...
# The multiple R-squared value is 0.588 and therefore 58.8% of the variation in ABUND is
# explained by LOGAREA

# WHICH ONE SHOULD YOU QUOTE?

# Multiple R-squared is simply the proportion of the variation in the response that this
# model accounts for. The problem with it is that it can only ever go up when you add
# another explanatory variable, even a completely useless one, because any extra
# variable will mop up a little variation by chance alone.

# Adjusted R-squared applies a penalty for the number of parameters estimated, so it can
# go down when you add a variable that isn't earning its place. That makes it the one to
# use whenever you want to compare models with different numbers of explanatory
# variables.
...
# important, that LOGAREA causes the change in ABUND, or that the model would predict
# well for a new set of forest patches. Equally, a low R-squared doesn't mean that an
# effect is unimportant, as plenty of real and useful effects sit inside very noisy
# systems.
...
# The normal Q-Q plot is used to assess normality of the residuals (top right). If
# perfectly normal then all residuals would lie exactly on the reference line. In
# reality this is never going to happen so we are looking for obvious large departures.
# In this plot they don't look too bad, although the lower quantiles do seem to deviate.
# All in all I would be cautiously optimistic.

# To check homogeneity of variance assumption we look at both the Residuals vs Fitted
# (top left) and the Scale-Location plots (bottom left). There doesn't appear to be any
# obvious patterns of the residuals in these plots, although you can see the two
# residuals associated with the two large forest patch areas are below the zero line
# (negative residuals) on the right hand side of these plots despite the log
# transformation. Perhaps something to bear in mind as we progress. But in short, the
# homogeneity of variance assumption looks ok.

# From the Residuals vs Leverage plot (bottom right) you can see that there are no
# residuals with a Cook's distance greater than 1. In fact they are all well below 0.5.
# If you want to produce a plot of just Cook's distance (perhaps this is clearer)
par(mfrow = c(1,1))
plot(loyn_lm, which = 4)

# Going back to the Residuals vs Leverage plot (bottom right) you can see three
# residuals that are somewhat unusual in terms of their leverage as they stick out a bit
# to the right compared to the rest of the residuals. Two of these residuals are our two
# large forest patch areas again. However, leverage still seems to be on the low side,
# so at least on the scale of the log transformation things look ok.
...
# if you used the natural log (i.e. log()) then you would use log() not log10()
...
# for those of you who are into plotting using the ggplot2 package, this is one of those
# occasions where things are a little simpler! Don't forget, you will need to install
# the ggplot2 package if you've never used it before
...
# note the use of the se.fit argument
pred_vals_se <- predict(loyn_lm, newdata = my_data, se.fit = TRUE)
...
lines(my_data$LOGAREA, pred_vals_se$fit + (1.96 * pred_vals_se$se.fit),
      lty = 2, col = "firebrick")
...
# now add the fitted lines and upper and lower 95% confidence intervals remember we only
# log10 transformed the LOGAREA variable so this is the only variable that we need to
# back-transform

lines(10^(my_data$LOGAREA), pred_vals_se$fit, lty = 1,col = "firebrick")
lines(10^(my_data$LOGAREA), pred_vals_se$fit + (1.96 * pred_vals_se$se.fit),
      lty = 2, col = "firebrick")
lines(10^(my_data$LOGAREA), pred_vals_se$fit - (1.96 * pred_vals_se$se.fit),
      lty = 2, col = "firebrick")
