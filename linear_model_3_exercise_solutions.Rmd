...
Those four plots all show the residuals against the *fitted values*. As you saw in the lectures, it is also worth plotting the residuals against each *explanatory variable* in turn. Extract the residuals from your model with the `resid()` function, store them in the `loyn` dataframe, and then plot them against `LOGAREA` (a scatterplot) and against `FGRAZE` (a boxplot). Add a horizontal line at zero to each using `abline(h = 0, lty = 2)` to make patterns easier to see. What can these two plots tell you that the previous four cannot?
...
```{r Q6c, eval=SOLUTIONS, echo=SOLUTIONS, results=SOLUTIONS, collapse=TRUE}
...
10\. And again, but with less help this time. Write up this analysis as a single paragraph for your thesis, referring to your figure from Question 9. No ingredient list this time, you decide what goes in.
