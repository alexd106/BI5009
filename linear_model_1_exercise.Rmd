...
### A note on P values and confidence intervals {#pval_note}
...
6\. What is the null hypothesis associated with the intercept? What is the null hypothesis associated with the slope? Do you reject or fail to reject these hypotheses? Now write those two findings out a second time, but in a completely different style: instead of saying which hypotheses you rejected, lead with the estimated values and their confidence intervals from Question 5. Put the two versions side by side. Which one tells a reader more, and what exactly does the second one tell them that the first one doesn't?
...
You will see two values reported, `Multiple R-squared` and `Adjusted R-squared`. Which of the two should you quote here, and why? (hint: think about what the 'adjustment' is adjusting for, and how many explanatory variables this model has).
...
> Bird abundance increased with forest patch area. A linear model of bird abundance against log~10~ transformed patch area estimated an increase of ______ birds (95% CI: ______ to ______) for every 1 unit increase in log~10~ area, which is a tenfold increase in patch area (*F*~___,___~ = ______, *p* ______). The model explained ______% of the variation in bird abundance (*n* = ______ patches).
