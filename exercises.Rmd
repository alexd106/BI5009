...
# derive the page names from the .Rmd sources rather than from rendered output,
# so this works whether the site is rendered whole or a page at a time
file_name <- list.files('.') %>%
  sort %>% 
  grep("^exercise_[0-9]\\.Rmd$", . , value=TRUE) %>%
  sub("\\.Rmd$", ".html", .)
...
# The PDFs must be written to the repo root, not into docs/: a full
# render_site() wipes the output directory and repopulates it, so anything
# written straight into docs/ during the render is destroyed. Writing here
# lets render_site copy them across. exercises/ is gitignored, so only the
# docs/ copy is committed
